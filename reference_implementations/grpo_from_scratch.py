# FROM https://github.com/mingyin0312/RLFromScratch/blob/main/grpo_train_from_scratch.py
import os
import re
import math
import random
...
for epoch in range(num_epochs):
    ...
    for step, (prompts, answers) in enumerate(
        ...
        input_ids = prompt_enc["input_ids"].to(
            local_rank
        )  # (B, prompt_len) and left_pad
        ...
        batch_attention_mask = (
            explore_generations != tokenizer.pad_token_id
        ).long()  # [batch_size * K, seq_len]
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                out_old = policy_model(
                    explore_generations, batch_attention_mask, use_cache=False
                )
        ...
        logits_old = logits_old[
            :, :-1, :
        ].contiguous()  # [batch_size * K, seq_len - 1, vocab_size]
        ...
        assert (
            batch_action_mask.shape[-1] == logprobs_old.shape[-1] + 1
            and batch_action_mask.shape == batch_attention_mask.shape
        )
        logprobs_old = logprobs_old.view(
            batch_size, K, -1
        )  # [batch_size, K, seq_len - 1]
        # --------------------------------------
        ...
        logprobs_ref = logprobs_ref.view(
            batch_size, K, -1
        )  # [batch_size, K, seq_len - 1]
        ...
        # Compute Advantages
        # --------------------------------------
        batch_responses_ids = explore_generations[
            :, prompt_len:
        ]  # (batch_size*K, response_length) right pad
        batch_responses = tokenizer.batch_decode(
            batch_responses_ids, skip_special_tokens=True
        )  # (batch_size*K, response_text_length)
        batch_answers = [
            extract_xml_answer(batch_responses[i]) for i in range(len(batch_responses))
        ]  # (batch_size*K, generated_answer_length) str
        ...
        batch_reward_scores = compute_reward(
            batch_answers, answers_K
        )  # (batch_size*K, 1)
        ...
        batch_advantages = (
            batch_rewards - batch_rewards.mean(dim=-1, keepdim=True)
        ) / batch_rewards.std(dim=-1, keepdim=True).clamp_min(1e-6)
        batch_advantages = batch_advantages.to(local_rank)  # (batch_size, K)
        assert batch_advantages.shape == (batch_size, K)
        batch_advantages = batch_advantages.unsqueeze(2).expand_as(
            logprobs_ref
        )  # [batch_size, K, seq_len - 1]
        ...
        for ppo_epoch in range(ppo_epochs):
            ...
            logprobs_new = logprobs_new.view(
                batch_size, K, -1
            )  # [batch_size, K, seq_len - 1]
            ...
            # --------------------------------------
            valid_mask = (
                batch_action_mask[:, :-1].contiguous().float().view(batch_size, K, -1)
            )  # [batch_size, K, seq_len - 1] # This is because the default pytorch average is not taken over the valid tokens
            # compute prob ratios
            ratio = torch.exp(
            ...
            individual_ppo_reward = torch.min(
                ratio * batch_advantages, ratio_clipped * batch_advantages
            )  # [batch_size, K, seq_len - 1]
            # compute KL penalty
            ...
            individual_kl_penality = (
                ratio_ref - ratio_ref_log - 1
            )  # [batch_size, K, seq_len - 1]
            # compute the overall GRPO loss
            sum_loss_ave_response = (
                individual_ppo_reward - kl_coef * individual_kl_penality
            ).sum(dim=-1)  # [batch_size, K]
            count_ave_response = valid_mask.sum(dim=-1)  # [batch_size, K]
            reward_ave_response = (
                sum_loss_ave_response / count_ave_response
            )  # [batch_size, K]
            ...
            if master_process:
                print(
                    f"grpo training loss at step {step} with ppo epoch {ppo_epoch} is: {loss_:.4f}"
                )
